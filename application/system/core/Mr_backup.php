<?php
define('ENVATO_API_KEY', '5f0kgad9c11avw9m84nhh8h7iit3273a');
...
function verify_purchase($purchase_code = NULL)
{
	if (is_null($purchase_code)) return false;

	$envato = new Envato(ENVATO_API_KEY);
	if (!$envato->verify_purchase($purchase_code))
		return false;
	else
		return $envato->get_response();
}
